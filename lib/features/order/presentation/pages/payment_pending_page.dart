...
import '/core/services/global_institute_pay_service.dart'; // untuk proses pembayaran
...
void _log(String msg) => debugPrint('[JajanSkuy/PaymentPending] $msg');
...
class _PaymentPendingPageState extends State<PaymentPendingPage>
  ...
  StreamSubscription<PaymentCallbackData>? _callbackSub; //listener callback
  ...
  void initState() {
    ...
    // 3. Periksa callback yang masuk saat cold start
    ...
    // 4. Subscribe stream callback (app berjalan di background/foreground)
  ...
  //5. Cegah memory leak dengan dispose()
  @override
  void dispose() {
    _log('dispose | orderId=${widget.order.id}');
    _callbackSub?.cancel(); //cegah memory leak
    WidgetsBinding.instance.removeObserver(this);
    context.read<OrderProvider>().stopPaymentPolling();
    super.dispose();
    /*Aturan penting: Setiap StreamSubscription.listen() HARUS diimbangi
      dengan .cancel() di dispose(). Jika tidak, callback bisa terpanggil
      setelah widget dihapus → crash atau bug tersembunyi.*/
  }

  //6. Lifecycle observer dengan WidgetBindingObserver: Deteksi kembali dari app lain
  @override
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    /*WidgetsBindingObserver adalah mixin yang memungkinkan widget
        mendengarkan perubahan lifecycle app:
        ●	resumed — app kembali ke foreground
        ●	paused — app masuk background
        ●	detached — app ditutup
    */
  }

  //7. Fungsi launch deeplink 
  Future<void> _launchGlobalInstitutePay() async {
    ...
    // Generate URL deeplink (dilakukan di sisi client, bukan dari backend) untuk buka aplikasi Dompet Kampus Global
    ...
    try {
      // Cek apakah aplikasi Dompet Kampus Global terpasang di handphone user
  ...
  Widget build(BuildContext context) {
    ...
    if (payStatus == PaymentCheckStatus.paid) {
      ...
      /*Kenapa `addPostFrameCallback` untuk cold start?
        Pada saat initState() berjalan, widget belum sepenuhnya mounted dan
        Navigator belum siap. addPostFrameCallback menunda eksekusi sampai
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        body: order.paymentMethod == 'virtual_account'
            ...
            : _GlobalInstitutePayBody( // Perubahan terbaru dari _GopayBody ke _GlobalInstitutePayBody
                order: order,
                payStatus: payStatus,
                formatPrice: _formatPrice,
